...
int CALLBACK NEnumFontNameProc(LOGFONTA* plf, TEXTMETRIC* /*ptm*/, INT /*nFontType*/, LPARAM lParam/**/)
{
	ST_FONTNAME* pFont = (ST_FONTNAME*)lParam;
	if (!pFont)
	{
		return 0;
	}
	std::string sFontName = plf->lfFaceName;
	if (sFontName.empty())
		return 1;
	if (sFontName.substr(0,1) == "@")
		return 1;
	HDC hdc=GetDC(NULL);
				...
				if(lpv) 
				{
					...
					if(dw != GDI_ERROR) 
					{
						...
						for(int i = 0; i < count; i++)
						{
							...
							{
								std::string  strEngName((LPCSTR)(lpv+stringOffset+rgNR[i].offset),rgNR[i].length);
								std::string strLogName = plf->lfFaceName;
								std::wstring wsEng = MultiByteToUnicode(strEngName.c_str());
								std::wstring wsZh = MultiByteToUnicode(strLogName.c_str());
								//printf("==========strEngName=%s,strLogName=%s\n",strEngName.c_str(),strLogName.c_str());
								//wprintf(L"=============wsEng=%s,wsZh=%s\n",wsEng.c_str(),wsZh.c_str());
								pFont->wsEn = wsEng;
								pFont->wsZh = wsZh;
								break;
							}
...
{
	HDC dc=::GetDC(NULL);
	std::string sName = UnicodeToMultiByte(wsFontName.c_str());
	ST_FONTNAME fontName;
	::EnumFontsA((HDC)dc,sName.c_str(), (FONTENUMPROCA)NEnumFontNameProc, (LPARAM)&fontName);
	wsFontNameEn = fontName.wsEn;
	wsFontNameZh = fontName.wsZh;
	::ReleaseDC(NULL, dc);
	if (wsFontNameEn.empty() || wsFontNameZh.empty())
	{
		return FALSE;
	}
	return TRUE;
...
BOOL CPdfTronPrivate::Pdf2EmbeddedFontPdf(const std::string sOutPdfPath,bool bDelOri)
{
	if (IsPdfA() || !m_pDoc)
	{
